...
const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    const { image, mimeType, feature } = JSON.parse(event.body) as RequestBody;
    ...
    if (!faceDetectionResponse.text.trim().toLowerCase().startsWith('yes')) {
         return {
            ...
            body: JSON.stringify({ error: "사진에서 얼굴을 찾을 수 없습니다. 정면이 보이는 사람 얼굴 사진으로 다시 시도해주세요." }),
        };
    }
    ...
    return {
      statusCode: 500,
      headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: `AI 서버와 통신 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요. (${errorMessage})` }),
    };
  }
};
